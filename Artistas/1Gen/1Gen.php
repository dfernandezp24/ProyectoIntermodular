...
$total_stmt = $pdo->prepare("SELECT COUNT(*) FROM artists WHERE generation = 1");
$total_stmt->execute();
$total_artists = $total_stmt->fetchColumn();
$total_pages = ceil($total_artists / $limit);

$stmt = $pdo->prepare("SELECT * FROM artists WHERE generation = 1 ORDER BY name ASC LIMIT $limit OFFSET $offset");
$stmt->execute();
