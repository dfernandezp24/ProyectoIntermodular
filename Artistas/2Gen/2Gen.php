...
$total_stmt = $pdo->prepare("SELECT COUNT(*) FROM artists WHERE generation = 2");
$total_stmt->execute();
$total_artists = $total_stmt->fetchColumn();
$total_pages = ceil($total_artists / $limit);

$stmt = $pdo->prepare("SELECT * FROM artists WHERE generation = 2 ORDER BY name ASC LIMIT $limit OFFSET $offset");
$stmt->execute();
...
$extra_css = 'Artistas/2Gen/css/lightMode.css';
$extra_css_tablet = 'Artistas/2Gen/css/lightModeTablet.css';
$extra_css_mobile = 'Artistas/2Gen/css/lightModeMovil.css';
