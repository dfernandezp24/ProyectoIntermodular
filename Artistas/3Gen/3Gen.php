...
$total_stmt = $pdo->prepare("SELECT COUNT(*) FROM artists WHERE generation = 3");
$total_stmt->execute();
$total_artists = $total_stmt->fetchColumn();
$total_pages = ceil($total_artists / $limit);

$stmt = $pdo->prepare("SELECT * FROM artists WHERE generation = 3 ORDER BY name ASC LIMIT $limit OFFSET $offset");
$stmt->execute();
...
$extra_css = 'Artistas/3Gen/css/lightMode.css';
$extra_css_tablet = 'Artistas/3Gen/css/lightModeTablet.css';
$extra_css_mobile = 'Artistas/3Gen/css/lightModeMovil.css';
